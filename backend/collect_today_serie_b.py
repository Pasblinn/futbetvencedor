#!/usr/bin/env python3
"""
🔥 COLETOR DE JOGOS DE HOJE - SÉRIE B
Busca jogos de hoje da Série B com odds reais da Bet365
"""
...
async def fetch_today_serie_b_matches():
    """Busca jogos de hoje da Série B"""
    print("🔍 Buscando jogos de HOJE da Série B...")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Encontrados {len(fixtures)} jogos de hoje na Série B")
            return fixtures
        else:
...
async def main():
    """Main function"""
    print("=" * 60)
    print("🔥 INICIANDO COLETA - SÉRIE B HOJE")
    print("=" * 60)

    # Buscar fixtures de hoje
    fixtures = await fetch_today_serie_b_matches()

    if not fixtures:
        print("❌ Nenhum jogo encontrado para hoje na Série B")
        return
    ...
    try:
        ...
        print(f"\n{'=' * 60}")
        print(f"✅ COLETA CONCLUÍDA - {len(fixtures)} jogos processados")
        print(f"{'=' * 60}")
